  ...
  public static func swiftFormat(
    ...
    guard output.isEmpty else {
      return sourceSwiftFormatUnmeasured(
        engine, subject, rules, "unexpected-output", "swift-format wrote stdout"
      )
    }
    ...
    for line in lines {
      guard
        let finding = sourceSwiftFormatFinding(
          Swift.String(line), engine: engine, files: files, rules: rules
        )
      else {
        return sourceSwiftFormatUnmeasured(
          engine, subject, rules, "malformed-output", Swift.String(line)
        )
      }
    ...
    guard (status == 0) == findings.isEmpty else {
      return sourceSwiftFormatUnmeasured(
        engine, subject, rules, "engine-status-mismatch", "status and findings disagree"
      )
    }
