extension Source.Repair {
  public struct Transaction: Sendable {
    ...
    public func apply(
      ...
      guard plan.subject == subject else {
        return .failure(.init(code: "stale-subject", detail: subject.identity))
      }
      guard plan.profile == profile else {
        return .failure(.init(code: "stale-profile", detail: profile.hex))
      }
      guard plan.sources == sources else {
        return .failure(.init(code: "stale-sources", detail: sources.hex))
      }
      ...
      for postcondition in plan.postconditions {
          ...
          case .success(let contents):
            guard digest(contents) == expected else {
              return rollback(
                ...
                after: .init(
                  code: "postcondition",
                  detail: "digest mismatch at \(path)"
                )
              )
            }
        ...
        case .absent(let path):
          guard !files.exists(path) else {
            return rollback(
              journal,
              after: .init(code: "postcondition", detail: "file exists at \(path)")
            )
          }
        }
      }
